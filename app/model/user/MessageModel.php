...
class MessageModel extends CacheModel
{
     ...
     * @var int
     */
    const CATLOG_SYSTEM = 1;
    
    /**
     * 系统公告
     * @var int
     */
    const CATLOG_ANNOUS = 2;
    ...
    {
        if (! isset(self::$_instances[$uid]) || ! is_object(self::$_instances[$uid])) {
     ...
     * @param string $text            
     */
    public function setMsg2User( $msg_uid, $catlog, $title, $content )
    {
        $insertInfo = [];
        $insertInfo['sender_uid'] = '0';
        $insertInfo['sender_nick'] = SITE_NAME;
    ...
    public function getMessagesByWhere( $uid, $where , $page,  $pagesize=20 ,$orderby='id' , $sort='DESC'  )
    {
        ...
        $fields = ' id,direction,title, sender_uid,sender_nick,receiver_uid,catlog,readed,create_time ';
    ...
    public function getLastUnreaded( ) 
    {
        ...
        if( $memFlag!==false )   return $memFlag;
     ...
     * @return number
     */
    public function getCountMessage( $uid  ,$where )
    {
    ...
    public function getUnreadedCountByCatlog( $catlog  )
    {
        $uid = $this->uid;
        $catlog = (int) $catlog;
        $key =  self::DATA_KEY.'unreaded_catlog_count/'.$catlog.'/'.$uid;
        $memFlag = $this->cache->get( $key );
        if( $memFlag!==false )
            return $memFlag;
    
        $readed = MessageModel::READED_NO;
        //$where = " Where receiver_uid='$uid' AND catlog='$catlog' ";
        $where = array('receiver_uid'=>$uid, 'catlog'=>$catlog);
        if ($readed != 2) {
            //$where .= " AND readed='$readed'  ";
            $where['readed'] = $readed;
        }
