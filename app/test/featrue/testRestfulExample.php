...
class testRestfulExample extends BaseTestCase
{
    ...
    public function testPatchUser()
    {
        ...
        $json = json_decode(  $curl->rawResponse, true );
        if( empty( $json ) ){
            $this->fail( 'PATCH '.ROOT_URL.'api/restful_example/users/1'.'  is not json data:'. $curl->rawResponse );
        }
        $this->assertEquals( '200', $json['ret'] );
        $this->assertNotEmpty( $json['data'], $curl->rawResponse );
    ...
    public function testDeleteUser()
    {
        ...
        $json = json_decode(  $curl->rawResponse, true );
        if( empty( $json ) ){
            $this->fail( 'DELETE '.ROOT_URL.'api/restful_example/users/1'.'  is not json data:'. $curl->rawResponse );
        }
        $this->assertEquals( '200', $json['ret'] );
        $this->assertNotEmpty( $json['data'],$curl->rawResponse );
