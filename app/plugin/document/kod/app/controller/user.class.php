...
class user extends Controller{
	...
	public function loginCheck(){
		...
		if(isset($_SESSION['kodLogin']) && $_SESSION['kodLogin']===true && $this->user){
			...
			if ($this->config['settingSystem']['autoLogin'] != '1') {
				...
				header('location:./kod_index.php?user/loginSubmit&name=guest&password=guest');
				exit;
			}
		}
	}
	...
	public function loginFirst(){
		...
		header('location:./kod_index.php?user/login');
		exit;
	}
	/**
	 * 退出处理
	 ...
	 * 登录数据提交处理；登陆跳转：
	 * 
	 * 自动登陆：kod_index.php?user/loginSubmit&name=guest&password=guest
	 * 登陆自动跳转：kod_index.php?user/login&link=http://baidu.com
	 * api登陆:kod_index.php?user/loginSubmit&login_token=ZGVtbw==|da9926fdab0c7c32ab2c329255046793
	 */
