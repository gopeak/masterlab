...
class Passport extends BaseUserCtrl
{
    ...
    public function register(  $email='',  $username='', $password='',$display_name='',$avatar=''  )
    {
        ...
        if( $ret[1]['uid'] ) {
            $this->send_active_email( $ret[1]['uid'] ,$email, $username );
     ...
     * @return array
     */
    private function send_active_email( $uid, $email='' ,$username='' )
    {
        $userModel = UserModel::getInstance();
     ...
     * @param $verify_code
     */
    public function active_email(  $email, $verify_code  )
    {
    ...
    public function display_reset_password(  $email, $verify_code  )
    {
        // 校验验证码
        $emailFindPasswordModel = new EmailFindPasswordModel();
    ...
    public function reset_password(  $email, $verify_code ,$password, $password_confirmation )
    {
        $userModel = UserModel::getInstance( '' );
        $user = $userModel->getByEmail( $email );
        if( !isset($user['email']) )
